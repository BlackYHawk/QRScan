 ...
 */
class QRPreview constructor(context: Context, attrs: AttributeSet) : View(context, attrs) {

    companion object {
        private val DefaultAnimationInterval = 10L         //刷新界面的时间
        private val DefaultOpaque = 0xFF                   //背景图片透明度
        private val DefaultTopTextSize = 18                //上面面字体大小
     ...
     * 扫描框上面的字
     */
    private var topHint: String? = null
    /**
     * 扫描框下面的字
     */
    ...
    init {
        ...
        topHint = context.getString(R.string.scan_qr_top)
        bottomHint = context.getString(R.string.scan_qr_bottom)
        paint = Paint()
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        val frame = CameraManager.get()?.getQRFramingRect() ?: return
    ...
    private fun drawTop(canvas: Canvas, frame: Rect) {
        //画扫描框上面面的字
        if (top) {
            paint?.setColor(hintColor)
            paint?.setTextSize(DefaultTopTextSize * density)
            paint?.setAlpha(0x5a)
            paint?.setTypeface(Typeface.create("System", Typeface.BOLD))
            paint?.setTextAlign(Paint.Align.CENTER)
            canvas.drawText(topHint, (widthPixels / 2).toFloat(), frame.top - DefaultTextPadding.toFloat() * density, paint)
        }
    }
